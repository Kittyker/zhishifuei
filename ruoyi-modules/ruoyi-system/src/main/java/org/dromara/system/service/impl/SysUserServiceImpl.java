...
public class SysUserServiceImpl implements ISysUserService, UserService {
     ...
     */
    @Override
    public SysUserVo registerUser(SysUserBo user, String tenantId) {
        user.setCreateBy(0L);
        user.setUpdateBy(0L);
        SysUser sysUser = MapstructUtils.convert(user, SysUser.class);
    ...
    @Override
   public SysUserVo queryUser(String openid, String phoneNumber){
        LambdaQueryWrapper<SysUser> queryWrapper = new LambdaQueryWrapper<SysUser>();
        queryWrapper.eq(SysUser::getOpenid, openid);
        if (StringUtils.isNotEmpty(phoneNumber)) {
            queryWrapper.or(wrapper -> wrapper.eq(SysUser::getPhonenumber, phoneNumber));
        }
        return baseMapper.selectVoOne(queryWrapper);
    }

    @Override
    public int updateUserOpenId(SysUserBo user) {
    ...
    public int updateUserByUserId(SysUserBo bo){
        SysUserVo sysUserVo = baseMapper.selectVoById(getUserId());
        if (Objects.isNull(sysUserVo)) return 0;
        if (org.apache.commons.lang3.StringUtils.isNotBlank(bo.getUserName())){
            sysUserVo.setUserName(bo.getUserName());
        }
        if (org.apache.commons.lang3.StringUtils.isNotBlank(bo.getNickName())){
            sysUserVo.setNickName(bo.getNickName());
        }
     return baseMapper.update(new LambdaUpdateWrapper<SysUser>().set(SysUser::getUserName, sysUserVo.getUserName()).eq(SysUser::getUserId, getUserId()));
